...
/// </summary>
internal sealed class GetContentByPageKeyUseCase : IUseCase<GetContentByPageKeyRequest, GetContentByPageKeyResponse>
{
    ...
    public async Task<GetContentByPageKeyResponse> HandleRequestAsync(GetContentByPageKeyRequest request)
    {
        PageContentOption contentOptions = _pageContentOptionProvider.GetPageContentOptionWithPageKey(request.PageKey);
        ArgumentException.ThrowIfNullOrWhiteSpace(contentOptions.DocumentId);
