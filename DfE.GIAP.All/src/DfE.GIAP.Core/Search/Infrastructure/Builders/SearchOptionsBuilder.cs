...
public sealed class SearchOptionsBuilder : ISearchOptionsBuilder
{
    ...
    {
        // Assign core search parameters based on the configured state of the builder.
        // These directly control search execution behaviour in Azure Search.
        _searchOptions.SearchMode = _searchMode;    // e.g., Any / All (logical operators between search terms)
        _searchOptions.Size = _size;                // Max number of results to return
        _searchOptions.Skip = _offset;              // Number of results to skip (for pagination)
        _searchOptions.IncludeTotalCount = _includeTotalCount;  // Whether to return the total count of matching documents

        // Adds fields and facets if provisioned.
        AddSearchFieldsIfAny();
        AddFacetsIfAny();
        // Applies any filters available.
        ApplyFiltersIfAny();

        // Return the fully configured SearchOptions instance to the caller.
        return _searchOptions;
    }

    /// <summary>
    /// Add configured search field names to the <see cref="SearchOptions.SearchFields"/>
    ...
    /// <summary>
    /// Add facet definitions to the <see cref="SearchOptions.Facets"/> collection if any.
    ...
    /// <summary>
    /// If one or more filters have been defined and we have a filter builder available,
    /// construct a filter expression string and assign it to SearchOptions.Filter.
    ...
    private void ApplyFiltersIfAny()
    {
        ...
        {
            // Project each filter into a SearchFilterRequest object for the builder to consume.
            ...
            // Use the builder to generate the final OData filter expression string.
