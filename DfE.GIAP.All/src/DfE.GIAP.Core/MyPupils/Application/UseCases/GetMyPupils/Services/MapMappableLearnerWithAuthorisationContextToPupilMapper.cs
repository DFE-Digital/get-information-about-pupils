...
internal sealed class MapMappableLearnerWithAuthorisationContextToPupilMapper : IMapper<MappableLearnerWithAuthorisationContext, Pupil>
{
    public Pupil Map(MappableLearnerWithAuthorisationContext input) =>
        new(
            identifier: new PupilId(Guid.NewGuid()), // TODO identifier should be hydrated on Learner or MyPupilList and surfaced up so we can uniquely identify a pupil in the list for Remove operations
            ...
            sex: new Sex(input.Learner.Sex.ToCharArray().Single()),
