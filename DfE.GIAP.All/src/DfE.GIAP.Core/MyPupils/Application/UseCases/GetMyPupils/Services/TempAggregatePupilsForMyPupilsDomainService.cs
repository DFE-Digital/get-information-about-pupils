...
/// <summary>
/// Methods used to extend the standard behaviour of IEnumerable types.
/// </summary>
public static class EnumerableExtensions
{
    /// <summary>
    /// Returns the correlation of specified TSource items for sequence
    /// order list and source list based on the delegated Id selector.
    /// This is obtained by performing an innner join on the sequence
    /// order list against the source list.
    /// </summary>
    /// <typeparam name="TSource">
    /// The type of object assigned to the incoming source collection.
    /// </typeparam>
    /// <typeparam name="TSequenceId">
    /// The type of identifier used to assign a sequenced order in the source list.
    /// </typeparam>
    /// <param name="sourceList">
    /// The IEnumerable collection of source objects on which to appy a sequenced ordering.
    /// </param>
    /// <param name="sequenceOrderList">
    /// The IEnumerable collection which lists the sequence on which to base
    /// the sequenced order to apply to the source enumerable. 
    /// </param>
    /// <param name="sourceListIdSelector">
    /// Delegate used to define the criteria on which to base the Id selection for the type specified.
    /// </param>
    /// <returns>
    /// The intersection of the source and sequence list items based on the delegated Id selector.
    /// </returns>
    public static IEnumerable<TSource> OrderEnumerableBySequence<TSource, TSequenceId>(
        this IEnumerable<TSource> sourceList,
        IEnumerable<TSequenceId> sequenceOrderList,
        Func<TSource, TSequenceId> sourceListIdSelector) =>
            sequenceOrderList.Join(
                sourceList,
                sequence => sequence,
                sourceListIdSelector,
                (_, source) => source);
}
