...
public class DfeClaimsEnricher : IClaimsEnricher
{
    ...
    public DfeClaimsEnricher(IDfeSignInApiClient apiClient)
    {
        ...
        CancellationToken ct)
    {
        ...
        string userId = providerPrincipal.FindFirst("sub")?.Value ?? string.Empty;
        string email = providerPrincipal.FindFirst("email")?.Value ?? string.Empty;
        claims.Add(new Claim(AuthClaimTypes.UserId, userId));
        claims.Add(new Claim(ClaimTypes.Email, email));

        // Organisation claim comes as JSON
        string orgJson = providerPrincipal.FindFirst("organisation")?.Value ?? "{}";
        JObject org = JObject.Parse(orgJson);
        string orgId = org["id"]?.ToString() ?? string.Empty;

        // Call DfE Sign-In API
        UserAccess? userAccess = await _apiClient.GetUserInfo(signInSettings.ServiceId, orgId, userId);
