...
public static class ValidationHelper
{
    public static bool UPNRegexValidator(string upn)
    {
        ...
        Match matchUPNFirstType = Regex.Match(upn, REGEX_UPN_TYPE_1);
        Match matchUPNSecondType = Regex.Match(upn, REGEX_UPN_TYPE_2);
    ...
    public static bool ULNRegexValidator(string uln)
    {
        ...
        Match matchULNType = Regex.Match(uln, REGEX_ULN_TYPE_1);
    ...
    public static bool IsValidUln(string uln)
    {
        ...
        int remainder = totalValueCounter % 11;
        if (remainder == 0)
        {
            return false;
        }
    ...
    {
        List<string> duplicates = upns.GroupBy(x => x)
                                 .Where(x => x.Count() > 1)
                                 .Select(y => y.Key)
                                 .ToList();
        return duplicates;
