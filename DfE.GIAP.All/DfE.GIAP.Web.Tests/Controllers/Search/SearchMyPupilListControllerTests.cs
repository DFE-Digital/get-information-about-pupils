...
namespace DfE.GIAP.Web.Tests.Controllers.Search
{
    [Trait("My Pupil List", "My Pupil List Controller Tests")]
    public class SearchMyPupilListControllerTests :
                                                    ...
                                                    IClassFixture<PaginatedResultsFake>
    {
        ...
        public SearchMyPupilListControllerTests(PaginatedResultsFake paginatedResultsFake, UserClaimsPrincipalFake userClaimsPrincipalFake)
        {
        ...
        [Fact]
        public async Task RemoveSelected_returns_mpl_page_with_confirmation_and_new_upn_list()
        {
            // arrange
            var upns = string.Join("\n", _paginatedResultsFake.GetValidLearners().Learners.Select(l => l.LearnerNumber));
            var upnArray = upns.FormatLearnerNumbers();
            ...
            _mockMplService.GetMyPupilListLearnerNumbers(Arg.Any<string>()).Returns(Task.FromResult(formattedMPLItems.Skip(1).Take(1)));
        ...
        [Fact]
        public async Task RemoveSelected_returns_mpl_page_with_sort_order_preserved()
        {
            // arrange
            ...
            _mockMplService.GetMyPupilListLearnerNumbers(Arg.Any<string>()).Returns(Task.FromResult(formattedMPLItems.Skip(1)));
