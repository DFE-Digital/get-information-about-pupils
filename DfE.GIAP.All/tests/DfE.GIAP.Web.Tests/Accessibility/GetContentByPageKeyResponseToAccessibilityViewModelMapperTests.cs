using DfE.GIAP.Core.Contents.Application.Models;
using DfE.GIAP.Core.Contents.Application.UseCases.GetContentByPageKeyUseCase;
using DfE.GIAP.Web.Controllers;
using DfE.GIAP.Web.Tests.TestDoubles;
using DfE.GIAP.Web.ViewModels;
using Xunit;

namespace DfE.GIAP.Web.Tests.Accessibility;
public sealed class GetContentByPageKeyResponseToAccessibilityViewModelMapperTests
{
    [Fact]
    public void Mapper_Throws_When_Input_Is_Null()
    {
        // Arrange
        GetContentByPageKeyResponseToAccessibilityViewModelMapper sut = new();

        // Act
        Action act = () => sut.Map(null!);

        // Assert
        Assert.Throws<ArgumentNullException>(act);
    }

    [Fact]
    public void Mapper_Throws_When_Input_Content_Is_Null()
    {
        // Arrange
        GetContentByPageKeyResponse response = new(Content: null);

        GetContentByPageKeyResponseToAccessibilityViewModelMapper sut = new();
        // Act
        Action act = () => sut.Map(response);

        // Assert
        Assert.Throws<ArgumentNullException>(act);
    }

    [Fact]
    public void Mapper_Maps_AccessibilityViewModel()
    {
        // Arrange
        Content content = ContentTestDoubles.Default();
        GetContentByPageKeyResponse contentResponse = new(content);
        GetContentByPageKeyResponseToAccessibilityViewModelMapper sut = new();

        // Act
        AccessibilityViewModel response = sut.Map(contentResponse);

        // Assert
        Assert.NotNull(response);
        Assert.Equal(response.Response, content);
    }
}
