...
public class SecurityHeadersMiddlewareTests
{
    ...
    public async Task InvokeAsync_adds_specified_headers()
    {
        ...
        RequestDelegate middleware = CreateMiddleware(new SecurityHeadersOptions { Add = headersToAdd });
    ...
    public async Task InvokeAsync_removes_specified_headers()
    {
        ...
        RequestDelegate middleware = CreateMiddleware(new SecurityHeadersOptions { Remove = headersToRemove });
    ...
    private static IConfiguration CreateMockConfiguration(SecurityHeadersOptions settings)
    {
        ...
        if (settings.Add is not null)
        {
            ...
            {
                inMemorySettings[$"{SecurityHeadersOptions.SectionName}:Add:{header.Key}"] = header.Value;
            }
        }
        ...
        if (settings.Remove is not null)
        {
            ...
            {
                inMemorySettings[$"{SecurityHeadersOptions.SectionName}:Remove:{i}"] = settings.Remove[i];
            }
        }
